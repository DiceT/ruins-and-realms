...
export class DiceForge {
  ...
  private calculateLabels(type: string, baseLabels: string[]): any[] {
    ...
    if (type === 'd10' || type === 'd100') {
      labels.unshift('')
    } else if (type === 'd14' || type === 'd16' || type === 'd18') {
      labels.unshift('')
    } else {
      labels.unshift('')
      labels.unshift('')
  ...
  private getGeometry(type: string, radius: number): THREE.Geometry {
    ...
    switch (type) {
      ...
      case 'd20':
        ...
        break
      case 'd14':
        geom = this.create_trapezohedron_geometry(radius, 7)
        break
      case 'd16':
        geom = this.create_trapezohedron_geometry(radius, 8)
        break
      case 'd18':
        geom = this.create_trapezohedron_geometry(radius, 9)
        break
    }
  ...
  private make_geom(vertices: any[], faces: number[][], radius: number, tab: number, af: number) {
    ...
    if (CANNON) {
      ...
      ;(geom as any).cannon_shape = new CANNON.ConvexPolyhedron({
        vertices: points as any,
        faces: facesC as any
      })
    }
    return geom
  ...
  private make_d10_geom(
    ...
    for (var i = 0; i < vertices.length; ++i) {
      ...
      const vertex = vertices[i].normalize().multiplyScalar(radius)
      ;(vertex as any).index = geom.vertices.push(vertex) - 1
    }
    for (var i = 0; i < faces.length; ++i) {
      const ii = faces[i],
  ...
  private create_d2_geometry(radius: number) {
    ...
    const getUV_Top = (r_ratio: number, i: number) => {
      ...
      return new THREE.Vector2(
        0.5 - 0.5 * Math.cos(ang) * r_ratio,
        0.5 + 0.5 * Math.sin(ang) * r_ratio
      )
    }
    ...
    for (let i = 0; i < segments; i++) {
      const next = (i + 1) % segments
      const i1 = topInnerStart + i
      const i2 = topInnerStart + next
      const o1 = topOuterStart + i
      const o2 = topOuterStart + next
    ...
    for (let i = 0; i < segments; i++) {
      ...
      const t2 = topOuterStart + next
      const b1 = botOuterStart + i
      const b2 = botOuterStart + next
    ...
    for (let i = 0; i < segments; i++) {
      const next = (i + 1) % segments
      const o1 = botOuterStart + i
      const o2 = botOuterStart + next
      const i1 = botInnerStart + i
      const i2 = botInnerStart + next
  ...
  private create_trapezohedron_geometry(radius: number, sides: number) {
    ...
    for (let k = 0; k < sides; k++) {
      const center = k * 2 // 0, 2, 4...
      const prev = (center - 1 + ringCount) % ringCount
      const next = (center + 1) % ringCount
