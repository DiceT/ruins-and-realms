...
export class RollController {
    ...
    public async repositionDice(targets: DiePositionRequest[], duration: number = 500) {
        ...
        const animations: { die: ActiveDie, startPos: THREE.Vector3, startQuat: THREE.Quaternion, endPos: THREE.Vector3, endQuat: THREE.Quaternion }[] = [];
        ...
        return new Promise<void>((resolve) => {
            const animate = () => {
                ...
                animations.forEach(anim => {
                    ...
                    anim.die.body.position.set(anim.die.mesh.position.x, anim.die.mesh.position.y, anim.die.mesh.position.z);
                    anim.die.body.quaternion.set(anim.die.mesh.quaternion.x, anim.die.mesh.quaternion.y, anim.die.mesh.quaternion.z, anim.die.mesh.quaternion.w);
                });
            ...
            if (targetFace) {
                ...
                if (die.type === 'd8') {
                    ...
                    switch (String(die.result)) {
                        ...
                        case '4': angleDeg = 45; break;
                        case '5': angleDeg = 45; break;
                        case '6': angleDeg = 225; break;
                        case '7': angleDeg = 135; break;
                        case '8': angleDeg = 315; break;
                        default: angleDeg = 0; break;
                    }

                    if (angleDeg !== 0) {
                        const twist = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 1, 0), angleDeg * (Math.PI / 180));
                        q.premultiply(twist);
                    }
                }
    ...
    private getDieValue(die: ActiveDie): string | number {
        ...
        if (isD4) {
            ...
            for (const fv of faceValues) {
                ...
                if (dot > maxDot) { maxDot = dot; closestFace = fv; }
            }
