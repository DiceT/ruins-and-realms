...
export class DungeonViewRenderer {
  ...
  public renderDungeonView(data: SeedGrowthState | DungeonData, settings: SeedGrowthSettings, showRoomNumbers: boolean = true, showWalkmap: boolean = false): void {
    ...
    if ('spine' in data) {
        ...
        // Check if data is pre-assembled (has corridors from DungeonAssembler)
        if (data.corridors && data.corridors.length > 0 && data.corridors[0].tiles.length > 0) {
            // Data is already assembled - just extract for rendering
            corridorTiles = data.corridors[0].tiles.map(t => ({ x: t.x, y: t.y }))
        } else {
            // Fallback: Run generation here (legacy path)
            // This should rarely happen if Controller calls Assembler first
            console.warn('[DungeonViewRenderer] No pre-assembled corridors, running generation inline')
        ...
        // Check if data is pre-assembled
        if ((data as any).corridors && (data as any).corridors.length > 0) {
            corridorTiles = (data as any).corridors[0].tiles.map(t => ({ x: t.x, y: t.y }))
        } else {
            // Fallback: Run generation here
            console.warn('[DungeonViewRenderer] No pre-assembled corridors for organic mode, running generation inline')
            const pathfinder = new CorridorPathfinder(settings.seed)
